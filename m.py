...
bot = telebot.TeleBot'7487256285:AAGoDF7gm-eQ_Wu-kEewnKnnxQqMUBgZuvE')
...
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Open in "append" mode
        file.write(f"Username: {@RUDRA_0846}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @RUDRA_0846 ❄."
        bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{@RUDRA_0846}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of @RUDRA_0846"
    bot.reply_to(message, response)
